...
def injpie(lis_y):
    """Plot a Pie Graph of that only injured happened"""
    ...
    injured_all_years = total(year2014, year2015, year2016, year2017, year2018, lis_y)
    plt.pie(injured_all_years, labels=lis_x, autopct="%1.1f%%", startangle=90, colors=colors)
    plt.axis("equal")
    plt.title("สถิติผู้บาดเจ็บทั้งหมดโดยเปรียบเทียบทุกปี", fontname='JasmineUPC', fontsize='25')
    plt.ylabel("หน่วย(คน)", fontname='JasmineUPC', fontsize='20')
    plt.xlabel("จำนวนผู้บาดเจ็บ", fontname='JasmineUPC', fontsize='20')
    plt.legend()
    plt.show()
